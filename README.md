...
- X/Twitter 关键人物动态浏览器源占位，默认关闭
- X 关键账号浏览器采集，默认关闭，可使用本机登录态
```
...
```text
adapter          rss / ticker_rss / search_rss / html_static / browser_dynamic
...
`browser_dynamic` 用 `playwright-core` 调本机 Chrome，适合动态页面兜底。默认关闭，因为 X/Twitter 等平台经常需要登录或 API 权限。若你有可信 RSS 转换源、RSSHub、内部抓取服务或官方 API 代理，优先用 RSS/API 方式接入。

X/Twitter 可以启用 `x_browser` adapter 作为第二条路径。它不会保存账号密码，只复用本机 Chrome profile。默认配置在 `config/sources.json`：
...
启用后，第一次运行会打开一个独立 Chrome profile。你在这个窗口里登录 X，之后监控会复用该登录态；每个账号之间默认等待 10 秒，整组账号默认 30 分钟跑一次。`.browser/` 已加入 `.gitignore`，登录态不会提交。
